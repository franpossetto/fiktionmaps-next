...
import type { Location } from "@/src/locations/location.domain"
...
export function createFictionsService(deps: FictionsServiceDeps) {
  async function getAll(): Promise<FictionWithMedia[]> {
    return deps.fictionsRepo.getAll()
  }

  async function getById(id: string): Promise<FictionWithMedia | null> {
    return deps.fictionsRepo.getById(id)
  }

  async function create(data: CreateFictionData): Promise<FictionWithMedia | null> {
    return deps.fictionsRepo.create(data)
  }

  async function update(id: string, data: UpdateFictionData): Promise<FictionWithMedia | null> {
