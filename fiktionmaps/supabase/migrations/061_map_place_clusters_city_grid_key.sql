-- Fix: cities near grid boundaries (e.g. London on the 0° meridian) were being
-- split into two clusters, each undercounting places, because grid buckets were
-- keyed on each place's own raw lat/lng. A city can straddle an integer-degree
-- boundary even though all its places belong together.
--
-- Fix: when a place has a resolved city_id, bucket it using that city's fixed
-- center (c.lat/c.lng) instead of the place's own coordinate, so every place in
-- the same city always lands in the same grid cell. Places without a resolved
-- city_id keep bucketing by their own coordinate (unchanged). Display position
-- (avg_lat/avg_lng, or the exact city center when city_count = 1) is untouched.
...
AS $$
  ...
  keyed AS (
    SELECT
      *,
      floor(COALESCE(city_lat, latitude) / p_grid_deg) AS gy,
      floor(COALESCE(city_lng, longitude) / p_grid_deg) AS gx
    FROM bounded
  ),
  bucketed AS (
    SELECT
