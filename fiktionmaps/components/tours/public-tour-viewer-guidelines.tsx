...
import { ArrowLeft, ChevronDown, ChevronUp, Copy, MapPin } from "lucide-react"
...
export function PublicTourViewerGuidelines({ slug }: PublicTourViewerProps) {
  ...
  const [isPlaceCardExpanded, setIsPlaceCardExpanded] = useState(false)
  ...
  const cityLabel =
    resolvedTour.tour.mode === "singleCity" && resolvedTour.tour.cityId
      ...
      : "Multi-city"
  const activeStop = resolvedTour.stops[activeStopIndex] ?? null
  const activeStopCity = activeStop ? cityNameById.get(activeStop.place.cityId) : undefined
  ...
  return (
    <div ref={scrollRef} className="relative h-full overflow-y-auto bg-background">
          <a
            href="#tour-main-guidelines"
            className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-50 focus:rounded-md focus:bg-background focus:px-3 focus:py-2 focus:text-sm focus:text-foreground focus:shadow-lg"
          >
            ...
            <PageStickyBar
              ...
              leading={
                <div className="flex min-w-0 items-center gap-3">
                  <Link
                    href="/tours"
                    className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-foreground transition-colors hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background"
          ...
          <main id="tour-main-guidelines" className="px-6 pb-10 pt-5 md:px-8">
          ...
          <div className="h-fit space-y-4">
            {activeStop && (
              <section className="rounded-lg border border-border/70 bg-card/20 p-3">
                <button
                  type="button"
                  onClick={() => setIsPlaceCardExpanded((current) => !current)}
                  aria-expanded={isPlaceCardExpanded}
                  aria-controls="place-details-panel-guidelines"
                  className="flex w-full items-center justify-between gap-3 text-left"
                >
                  <div className="flex min-w-0 items-center gap-3">
                    <div className="relative h-14 w-20 shrink-0 overflow-hidden rounded-md border border-border/70">
                      <Image
                        src={activeStop.place.coverImage}
                        alt={activeStop.place.name}
                        fill
                        sizes="80px"
                        className="object-cover"
                      />
                    ...
                    <div className="min-w-0">
                      <p className="truncate text-sm font-semibold text-foreground">{activeStop.place.name}</p>
                      <p className="truncate text-xs text-muted-foreground">
                        {activeStopCity || cityLabel}
                        {activeStop.place.address ? ` • ${activeStop.place.address}` : ""}
                      </p>
                    </div>
                  </div>
                  {isPlaceCardExpanded ? (
                ...
                {isPlaceCardExpanded && (
                  <div id="place-details-panel-guidelines" className="mt-3 space-y-2">
                    <p className="text-xs uppercase tracking-[0.12em] text-muted-foreground">
                      Creator recommendation
                    </p>
                    <p className="text-sm text-foreground">
                      {activeStop.note?.trim() || "No recommendation was added for this stop."}
                    </p>
                    {activeStop.place.visitTip && (
                      <p className="text-xs text-muted-foreground">
                        <span className="font-semibold text-foreground">Local tip:</span>{" "}
                        {activeStop.place.visitTip}
                      </p>
                    )}
                    {activeStop.place.description && (
                      <p className="text-xs text-muted-foreground">{activeStop.place.description}</p>
                    )}
                  </div>
                )}
              </section>
            ...
            <div className="mb-2 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-foreground">Places playlist</h2>
              </div>
              <span className="text-xs text-muted-foreground">{resolvedTour.stops.length} tracks</span>
            </div>
            ...
            <div className="space-y-1.5 lg:hidden">
              {resolvedTour.stops.map((stop, index) => {
                ...
                return (
                  ...
                  >
                    <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-secondary text-[10px] font-semibold text-foreground">
                      {index + 1}
                    </div>
            ...
            <div className="hidden divide-y divide-border/60 overflow-hidden rounded-lg border border-border/70 bg-card/20 lg:block">
              {resolvedTour.stops.map((stop, index) => {
                const stopCity = cityNameById.get(stop.place.cityId)
                const isActive = index === activeStopIndex
                ...
                return (
                  <button
                    ...
                    className={`group flex w-full items-center gap-3 px-3 py-2 text-left transition-colors ${
                      isActive ? "bg-primary/10" : "hover:bg-card/45"
                    }`}
                  >
                    ...
                    <div className="relative h-10 w-14 shrink-0 overflow-hidden rounded-md border border-border/70">
                      <Image
                        src={stop.place.coverImage}
                        alt={stop.place.name}
                        fill
