...
export type CreditsContributeDoneVariant = "pending" | "approved"

export interface CreditsContributeDoneViewProps {
  variant: CreditsContributeDoneVariant
  fictionTitle: string
  personName: string
  roleLabel: string
  fictionHref: string
...
export function CreditsContributeDoneView({
  variant,
  fictionTitle,
  personName,
  roleLabel,
  ...
  className,
}: CreditsContributeDoneViewProps) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-6 w-full max-w-[280px] space-y-1 text-sm">
        <p className="font-semibold text-foreground">{fictionTitle}</p>
        <p className="text-muted-foreground">{t("doneCreditLabel", { role: roleLabel, name: personName })}</p>
      </div>

      {isApproved ? (
        <Button asChild className="mt-5 h-10 w-full max-w-[280px] rounded-lg text-sm font-semibold">
          <Link href={fictionHref}>{t("doneViewFiction")}</Link>
        </Button>
      ) : null}
